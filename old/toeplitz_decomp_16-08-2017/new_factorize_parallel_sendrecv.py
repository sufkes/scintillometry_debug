        ...
        for k in range(self.kCheckpoint + 1,n*(1 + pad)):
            ...
            if self.rank == 0:
                if k==1:
                    ...
                    print "Loop "+str(k-1)+" time = "+str(self.end_time-self.start_time)+"\nLoop {0}".format(k)
                    self.start_time = MPI.Wtime()
    ...
    def __new_block_update(self, X2, sb1, eb1, u1, e1,s2, sb2, eb2, u2, e2, S, m, p_eff):
        for b in self.blocks:
            ...
            if b.rank == s2: # rank s2=k sends to rank 0.
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=0, sendtag=21*num + b.getWork2(), recvbuf=data, source=0, recvtag=21*num + b.getWork2())
                del data
                ###################
            ...
            if b.rank == 0: # rank 0 receives from and sends to rank s2=k.
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=s2, sendtag=21*num + b.rank, recvbuf=data, source=s2, recvtag=21*num + b.rank)
                del data
                ###################
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=s2, sendtag=24*num + b.rank, recvbuf=data, source=s2, recvtag=24*num + b.rank)
                del data
                ###################                
            ...
            if b.rank == s2: # rank s2=k receives from rank 0.
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=0, sendtag=24*num + b.getWork2(), recvbuf=data, source=0, recvtag=24*num + b.getWork2())
                del data
                ###################
    ...
    def __block_update(self, X2, sb1, eb1, u1, e1,s2, sb2, eb2, u2, e2, S, method):
        def yty2():
            ...
            for b in self.blocks: # ranks k+1, ..., min(n-1+k, 2n-1) send to (rank-k)
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=b.getWork2()%self.size, sendtag=21*num + b.getWork2(), recvbuf=data, source=b.getWork2()%self.size, recvtag=21*num + b.getWork2())
                del data
                ###################
            ...
            for b in self.blocks: # ranks k+1, ..., min(n-1+k, 2n-1) receive from and send to (rank+k)
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=b.getWork1()%self.size, sendtag=21*num + b.rank, source=b.getWork1()%self.size, recvtag=21*num + b.rank)
                del data
                ###################
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=b.getWork1()%self.size, sendtag=28*num + b.rank, source=b.getWork1()%self.size, recvtag=28*num + b.rank)
                del data
                ###################
            ...
            for b in self.blocks: # ranks k+1, ..., min(n-1+k, 2n-1) receive from (rank-k)
                ...
                ###################
                data=np.array([0])
                self.comm.Sendrecv(data, dest=b.getWork2()%self.size, sendtag=28*num + b.getWork2(), source=b.getWork2()%self.size, recvtag=28*num + b.getWork2())
                del data
                ###################
