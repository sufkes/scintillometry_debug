...
class ToeplitzFactorizor:
    ...
    def addBlock(self, rank):
        ...
        if k!= 0:
            ...
            if rank >= self.n:
                m = self.m
                b.createA(np.zeros((m,m), complex, order='F')) # Assigns A1 and A2 for current instance of Block.
    ...
    def __setup_gen(self): # Sets up generator matrix A.
        ...
        for b in self.blocks:
            if b.rank < self.n:
                b.createA(np.asfortranarray(b.getT().dot(cinv)))
    ...
    def __block_reduc(self, s1, e1, s2, e2, m, p, method, k):
        ...
        for sb1 in range (0, m, p):
            ...
            if method == WY1 or method == WY2:
                ...
                S = np.zeros((p, p), complex, order='F' )
...
#                print "Fortran-order (ztrtri 1): "+str(invT.flags['F_CONTIGUOUS'])
...
#                print "Fortran-order (ztrtri 2): "+str(invT.flags['F_CONTIGUOUS'])
                M = M.dot(ztrtri(invT[:p_eff,:p_eff])[0]) # invert an upper triangular matrix
