...
from scipy.linalg.blas import zgeru, zherk, zgemm, zaxpy
...
#                A2[s:, :m] = A2[s:,:m] + M.dot(X2)
    ...
    def __aggregate(self,S,  X2, beta, m, j, p_eff, method):
        ...
        invT[:p_eff,:p_eff] = zherk(1.0, X2[:p_eff, :m].T, beta=-1.0, c=np.identity(p_eff,complex).T, trans=2, lower=1, overwrite_c=0)
        invT[:p_eff,:p_eff] = invT[:p_eff,:p_eff].T
...
#            A1[start:end,j] -= beta*v # size decreases with j.
            ...
            if start != end:
                A1[start:end,j] = zaxpy(x=v.T, y=A1.T[j,start:end], a=-beta).T
